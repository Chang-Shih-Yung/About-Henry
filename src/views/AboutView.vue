<template>
  <div class="h-full flex flex-col">
    <div class="mb-2 shrink-0 flex items-center gap-3">
      ...
      <div class="ml-auto flex items-center gap-2">
        <div class="flex gap-1 bg-muted p-1 rounded-lg">
          <button
            v-for="m in modes" :key="m.key" @click="mode = m.key"
            class="px-3 py-1 text-xs rounded-md font-medium transition-colors"
            :class="mode === m.key ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'"
          >{{ m.label }}</button>
        </div>
    ...
    <SectionEditor v-if="loaded" ref="editorRef" class="flex-1 min-h-0" filename="About.md" :initial-content="content" :mode="mode" />
