<template>
  <div class="h-full flex flex-col">
    <div class="mb-2 shrink-0 flex items-center gap-3">
      <h1 class="text-xl font-bold text-foreground shrink-0">MES</h1>
      <p class="text-sm text-muted-foreground hidden sm:block">
        編輯 <code class="text-xs bg-muted px-1.5 py-0.5 rounded font-mono">content/MES.md</code>
      </p>
      <div class="ml-auto flex items-center gap-2">
        ...
        <Button v-if="!isViewer.value" size="sm" class="h-7 px-3 text-xs"
          :disabled="!isOnline || (status !== 'dirty' && status !== 'error')"
    ...
    <SectionEditor v-if="loaded" ref="editorRef" class="flex-1 min-h-0"
      filename="content/MES.md"
      :initial-content="content"
      :mode="isViewer.value ? 'preview' : mode" />
...
const status = computed(() => fileStatuses['content/MES.md'] || 'idle')

watch(isOnline, (online) => { if (!online) mode.value = 'preview' })

onMounted(async () => {
  try {
    const res = await authFetch('/api/load?filename=content/MES.md')
