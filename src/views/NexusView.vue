<template>
  <div class="h-full flex flex-col">
    <div class="mb-2 shrink-0 flex items-center gap-3">
      <h1 class="text-xl font-bold text-foreground shrink-0">Nexus</h1>
      <p class="text-sm text-muted-foreground hidden sm:block">
        編輯 <code class="text-xs bg-muted px-1.5 py-0.5 rounded font-mono">content/Nexus.md</code>
      </p>
    ...
    <SectionEditor v-if="loaded" ref="editorRef" class="flex-1 min-h-0" filename="content/Nexus.md" :initial-content="content" :mode="isViewer.value ? 'preview' : mode" />
...
const status = computed(() => fileStatuses['content/Nexus.md'] || 'idle')
